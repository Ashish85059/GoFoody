...
const Navbar = () => {
  ...
  useEffect(() => {
    // Ensure data is loaded before accessing its properties
    if (data) {
    ...
    <div>
      <nav className="navbar navbar-expand-lg navbar-dark bg-success">
        <div className="container-fluid">
          <Link className="navbar-brand fs-1 fst-italic" to="/">GoFoody</Link>
            ...
            {localStorage.getItem("authToken") ?
              <div>
                <div className='btn bg-white text-success mx-2' onClick={() => setCartView(true)}>My Cart{" "}
                  {data && <Badge pill bg="danger">{data.length}</Badge>}
                </div>
